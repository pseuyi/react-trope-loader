...
const getPhrase = () => {
  // once all phrases are exhausted, choose from entire set
  if (len < 1) {
    len = phrases.length
  }
  // choose a random idx from phrases that have not been selected
  let rIdx = Math.floor((Math.random() * len--))
  // take phrase out and place it at the end
  let phrase = phrases.splice(rIdx, 1)[0]
  phrases.push(phrase)
